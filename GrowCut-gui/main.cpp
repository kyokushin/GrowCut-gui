...
struct MouseEventContainer {
	MouseEventContainer()
		:line_tickness(20),drawing(false),
			...
			for( int h=0; h<label.cols; h++ ){
				...
				for( int neigh_y=-1; neigh_y<=1; neigh_y++ ){
					...
					for( int neigh_x=-1; neigh_x<=1; neigh_x++ ){
						...
						//if( label.at<double>(i,h) != label.at<double>(i+neigh_y, h+neigh_x ))
							converged++;
						...
						/*
						if( G * strength.at<double>( i, h ) <= strength.at<double>( i+neigh_y, h+neigh_x ) ) continue;

						next_label.at<double>( i+neigh_y, h+neigh_x ) = label.at<double>( i, h );
						next_strength.at<double>( i+neigh_y, h+neigh_x ) = G * strength.at<double>( i, h );
						*/
					}
				}
			}
